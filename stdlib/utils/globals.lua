    ...
    while true do
        ...
        if trace then
            ...
            if (trace.what == 'Lua' or trace.what == 'main') and not ignored[trace.name] then
                local cur = trace.source:gsub('.*', ''):gsub('.*/Factorio%-Stdlib', '')
...
-- Defines Mutates
require('stdlib/utils/defines/color')
require('stdlib/utils/defines/anticolor')
require('stdlib/utils/defines/lightcolor')
require('stdlib/utils/defines/time')
...
function STDLIB.create_stdlib_globals(files)
    ...
    for glob, path in pairs(files) do
        rawset(_ENV, glob, require('/' .. (path:gsub('%.', '/')))) -- extra () required to emulate select(1)
    end
end
