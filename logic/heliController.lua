...
function assignHeliController(owner, heli, target, targetIsPlayer)
    if not storage.heliControllers then
        storage.heliControllers = {} -- Ensure initialization
    end

    local oldControllerIndex = getHeliControllerIndexByHeli(heli)
    ...
    table.insert(storage.heliControllers, heliController.new(owner, heli, target, targetIsPlayer))
end
...
heliController = 
{
    new = function(player, heli, target, targetIsPlayer)
        ...
        table.insert(storage.heliControllers, obj)
    ...
    destroy = function(self)
        ...
        if storage.heliControllers then
            local index = searchIndexInTable(storage.heliControllers, self, "heli")
            if index then
                table.remove(storage.heliControllers, index)
            end
        end
    ...
    stopAndDestroy = function(self)
        if self.driverIsBot then
            self.targetIsPlayer = false -- Ensure the flag is cleared
        ...
        elseif self.driverIsBot and curPassenger and curPassenger.valid then
            ...
            if self.targetIsPlayer then
                if self.targetPlayer.valid then
                    ...
                    self.targetIsPlayer = false -- Invalid player target, clear the flag
                    self:stopAndDestroy()
                    return
                end
            end
