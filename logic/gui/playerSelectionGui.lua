...
playerSelectionGui =
{
    ...
    OnGuiClick = function(self, e)
        ...
        if name:match("^" .. self.prefix .. "btn_.+$") then
            local btnData = searchInTable(self.guiElems.btns, e.element, "btn")
            if e.shift then
                self.manager:OnChildEvent(self, "selectedPosition", btnData.player.position)
            else
                self.manager:OnChildEvent(self, "selectedPlayer", btnData.player)
            end
